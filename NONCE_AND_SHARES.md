...
| a miner gets `too many failed authorizations from this address` | it failed `mining.authorize` `auth_max_failures` times inside `auth_fail_lockout_sec` — almost always a mistyped payout address retried on every reconnect | fix the username; the lockout clears itself when the window passes, and a successful authorize clears it at once |
